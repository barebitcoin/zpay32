...
import (
	...
	// "github.com/barebitcoin/zpay32/lnwallet/chainfee"
...
type DynPropose struct {
	...
	// transaction.
	KickoffFeerate fn.Option[btcutil.Amount]
...
func (dp *DynPropose) Encode(w *bytes.Buffer, _ uint32) error {
	...
	dp.KickoffFeerate.WhenSome(func(kickoffFeerate btcutil.Amount) {
...
func (dp *DynPropose) Decode(r io.Reader, _ uint32) error {
	...
	if val, ok := typeMap[DPKickoffFeerate]; ok && val == nil {
		dp.KickoffFeerate = fn.Some(
			btcutil.Amount(kickoffFeerateScratch),
		)
	}
